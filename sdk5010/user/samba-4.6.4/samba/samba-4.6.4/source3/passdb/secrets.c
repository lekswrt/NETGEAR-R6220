...
//#define SC_SECURITY_ENHANCE
#ifdef SC_SECURITY_ENHANCE
#define SC_CFPRINTF(fmt, args...)    do{  if(!access("/tmp/sc_dbg", F_OK)) { FILE *fp=fopen("/dev/console", "a+"); if(fp) {fprintf(fp, "[%s::%s():%d] ", __FILE__, __FUNCTION__, __LINE__);fprintf(fp, fmt, ##args);fclose(fp);}}}while(0)

static int sc_pw_reverse(char *data, int size, int enc)
{
	int i = 0;
	char tmp;
	if(!data)
...
#if 0
	SC_CFPRINTF("before %s:\n", enc ? "enc":"des");
	for(i=0; i<size; i++)
		SC_CFPRINTF("%02x", data[i]);
	SC_CFPRINTF("\n");
#endif
...
static void sc_dump_data(const char *data, int size)
{
	int i = 0, len = 0;
	char buf[512];
	if(!data || !size || access("/tmp/sc_dbg", F_OK) != 0) 
		return;
	for(i=0; i<size; i++)
	{
		len += snprintf((char *)buf + len, sizeof(buf)-len, "%s%02x", (i%16 == 0) ? "\n":" ", data[i] & 0xff);
	}
	buf[sizeof(buf)-1] = 0;
	if(len < sizeof(buf))
		buf[len-1] = 0;
...
#ifdef SC_SECURITY_ENHANCE
	SC_CFPRINTF("%s: fetch key:%s, size:%d\n", __func__, key, dbuf.dsize);
	if(key && !strcmp(key, "SECRETS/SID/READYSHARE"))
	{
		sc_pw_reverse(result, dbuf.dsize, 0);
		sc_dump_data(result, dbuf.dsize);
	}
#endif
...
#ifdef SC_SECURITY_ENHANCE
	SC_CFPRINTF("%s: store key:%s, size:%d\n", __func__, key, size);
	if(key && !strcmp(key, "SECRETS/SID/READYSHARE"))
	{
		sc_dump_data(data, size);
		sc_pw_reverse(data, size, 1);
	}		
#endif
