...
static void ext4_put_super(struct super_block *sb)
{
	...
	if (sbi->s_journal) {
		err = jbd2_journal_destroy(sbi->s_journal, block_device_ejected(sb));
		sbi->s_journal = NULL;
		if (err < 0)
...
failed_mount_wq:
	ext4_release_system_zone(sb);
	if (sbi->s_journal) {
		jbd2_journal_destroy(sbi->s_journal, block_device_ejected(sb));
		sbi->s_journal = NULL;
	}
	percpu_counter_destroy(&sbi->s_freeblocks_counter);
	percpu_counter_destroy(&sbi->s_freeinodes_counter);
...
out_journal:
	jbd2_journal_destroy(journal, block_device_ejected(sb));
out_bdev:
