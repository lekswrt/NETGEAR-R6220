...
extern int block_device_ejected(struct super_block *sb); /* see defines and comments in fs/ext4/super.c */
...
static void hfsplus_put_super(struct super_block *sb)
{
	...
	if (!(sb->s_flags & MS_RDONLY) && HFSPLUS_SB(sb).s_vhdr) {
		...
		if (!block_device_ejected(sb)) {
			...
			printk("*** HFS+ backing device is ejected, do not write back! ***\n");
		}
	}
