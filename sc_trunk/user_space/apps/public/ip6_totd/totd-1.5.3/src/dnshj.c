...
char *dnshj_parse_and_alloc_addr (char *caddr, int port, int *sa_len_ret) {
...
void dnshj_init (void) {
	...
	for (gl_tmp = T.dnshj_list->next; gl_tmp->list_data; gl_tmp = gl_tmp->next) {
		...
		if (sa_p) {
			...
			if (sa_p_len == sizeof(struct sockaddr_in6)) 
				dnshj_p->qtype = 0x1c;
			else
				dnshj_p->qtype = 0x01;
		} else {
 ...
 */
void *dnshj_select ( char *domain, int qtype) {
    ...
    for (gl = T.dnshj_list->next; gl->list_data; gl = gl->next)
	{
		...
		if (strcmp(domain, dnshj_tmp->dns_hostname) || qtype != dnshj_tmp->qtype)
		{
			continue;
		}
		else
		{
			...
			struct sockaddr_in *v4addr = NULL;	
		    syslog (LOG_DEBUG, "find dnshj %s", dnshj_tmp->hostname);
			if (qtype == 0x1c)
			{
			...
			{
				v4addr = (struct sockaddr_in *)dnshj_tmp->sa;
				pt = (void *)(&v4addr->sin_addr);
			}
#endif		    
		    break;
		}
