...
#include "libscm_wl.h"
#define MAX_ENTRY_NUM   253
int check_if_guest(struct dhcpMessage *oldpacket)
{
	
	char climac[64]="";
	int m=0;
	struct wlan_info wlan_devices_info[MAX_ENTRY_NUM];
	memset((void *)wlan_devices_info,0,sizeof(wlan_devices_info));
	get_assoc_wlan_mac_iface(&(wlan_devices_info[0]));
	sprintf(climac,"%02X:%02X:%02X:%02X:%02X:%02X",oldpacket->chaddr[0],oldpacket->chaddr[1], oldpacket->chaddr[2], oldpacket->chaddr[3], oldpacket->chaddr[4], oldpacket->chaddr[5]);
	for(m=0;wlan_devices_info[m].wlan_iface != -1;m++){
		if(strcasecmp(climac,(char*)wlan_devices_info[m].w_mac_e)==0){
			if(wlan_devices_info[m].ap_index!=0){
				return 1;
			}
		}	
	}	
...
#endif
#ifdef GUEST_LEASE_TIME
	if(check_if_guest(oldpacket)==1)
		lease_time_align = server_config[ifid].lease_gst;
	else 
#endif 
