...
#ifdef GUEST_LEASE_TIME
		if ((option_g = find_option(server_config[i].options, DHCP_LEASE_TIME_GUEST))) {
			memcpy(&server_config[i].lease_gst, option_g->data + 2, 4);
			server_config[i].lease_gst = ntohl(server_config[i].lease_gst);
		}
		else server_config[i].lease_gst = LEASE_TIME;
#endif
