 	...
 	* Porting from D7000: Nick*/
		...
		if(isdigit(*pt))/*start from next space*/
		{
			...
			if(strstr(pt1,"5.7.0 Must issue a STARTTLS command first") || 
				(strstr(conf.mailhost, "smtp.qq.com") && strstr(pt1,"Must issue a STARTTLS command first")))
			{
				do_starttls = 1;
				return;
			}
...
#ifdef _SC_DEBUG_ENABLE_
{
	...
	if (do_ssl == 1 && (
						(strstr(conf.mailhost, "smtp.qq.com") && (conf.mailport==587)) || 
						(strstr(conf.mailhost, "smtp.gmail.com") && (conf.mailport==587)) || 
						strstr(conf.mailhost, "smtp.live.com")
						)
	)
	{
		SC_CFPRINTF("send with STARTTLS\n");
		do_ssl = 0;
		do_starttls = 1;
		need_auth = 0;
	} else if (do_ssl == 1)
	{
		SC_CFPRINTF("send with SSL\n");
	} else
	{
		SC_CFPRINTF("send with CLEAR\n");
	}
	...
	if (do_starttls && strstr(conf.mailhost, "smtp.live.com"))
	{
		// smtp.live.com not send 
	} else
	{
		if(conf.from_addr)
			chat("MAIL FROM: <%s>\r\n", conf.from_addr);
