...
*/
    ...
    */
    ...
    shutdown(connect, SHUT_RDWR); //SHUT_RDWR, SHUT_WR. send fin even the fd ref not 0. 
    close(connect);
...
void tcp_handle_request()
{
    ...
    close(arg->connect); // the close send fin only if the ref counter 0. (if need fin in child process before father process call close, use shutdown.)
    						// must do close in father process since if not, fd number will increase. 
    free(arg);
#endif
