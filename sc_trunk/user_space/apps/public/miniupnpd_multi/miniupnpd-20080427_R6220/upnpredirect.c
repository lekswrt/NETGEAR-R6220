...
#include "libcnapt.h"
#endif
#define MAX_UPNP_PORTMAP_ENTRY	253
#ifdef ENABLE_LEASEFILE
...
int lease_file_add( unsigned short eport, const char * iaddr, unsigned short iport, int proto, const char * desc, int enabled)
{
	...
	if (upnp_get_portmapping_number_of_entries() >= MAX_UPNP_PORTMAP_ENTRY) 
		return 0;
        ...
        unsigned int leaseduration)
{
	...
	else if (upnp_get_portmapping_number_of_entries() >= MAX_UPNP_PORTMAP_ENTRY)
	{
		SYSLOG(LOG_INFO, "The number of entries reach the max\n");
		return -2;
	}
	else
    {
