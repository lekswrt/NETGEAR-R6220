...
AddPortMapping(struct upnphttp * h, const char * action)
{
	...
	char s='.';
    const char *pt=GetLANIPRouterList();
	char lanip_fliter[20];
	...
	if(int_ip)
	{
		memset(lanip_fliter,0,sizeof(lanip_fliter));
    	char *fliter=strrchr(pt,s);
    	size_t length=strlen(pt)-strlen(fliter)+1; 
