...
int main(int argc,char *argv[]){
	...
	if (argv[1]==NULL)
		...
		if(readycloud_nvram)
		{
			system("/bin/echo readycloud_nvram try commit while set! > /dev/console"); // when use readycloud_nvram set, they have commit later by the user, so that no need commit now.
		} else
		{
			nvram_commit();
		}
